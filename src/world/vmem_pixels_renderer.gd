...
func _ready() -> void :
	E.follow_events(self, [
		E.vd_vmem_pixels_editing_toggle, 
		E.vd_vmem_pixels_images_change, 
		E.vd_vmem_pixels_blink, 
	])
	L.sig = E.connect("mi_mode_change_requested", self, "_on_mi_mode_change_requested")
	set_process(false)
func _ev_vd_vmem_pixels_editing_toggle(_mode: int, _args: Dictionary) -> void :
	var p_is_editing: bool = _args[E.vd_vmem_pixels_editing_toggle.p_is_editing]
	visible = p_is_editing
func _ev_vd_vmem_pixels_images_change(_mode: int, _args: Dictionary) -> void :
	var p_image_renderer: Image = _args[E.vd_vmem_pixels_images_change.p_image_renderer]
...
func _process(delta: float) -> void :
	...
	if blinking_countdown < 0:
		set_process(false)
		$TextureRect.modulate = Color.white if not is_simulating else Color(1, 1, 1, 0)
		return
	var factor: float = sin(blinking_accumulator * 10.0)
	factor = (factor + 1) / 2.0
	$TextureRect.modulate = Color.white.linear_interpolate(Color(0.4, 0.4, 0.4, 1), factor)
