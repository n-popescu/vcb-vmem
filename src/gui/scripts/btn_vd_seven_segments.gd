...
enum SB{DIGITS, POS_X, POS_Y, SCALE}
...
func _ready():
	...
	L.sig = ButtonContainer.get_node("HBoxDigits/SBDigits/SpinBox").connect("value_changed", self, "_on_spinbox_value_changed", [SB.DIGITS])
...
func update_screen_settings() -> void :
	E.emit_signal("vd_display_value_settings_changed", is_visible, digits, pos_x, pos_y, scale)
