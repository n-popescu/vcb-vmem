...
func _qr_ed_timer_interval() -> int:
	return SB.public_get_int_value()
func _ev_fs_project_change(_mode: int, _args: Dictionary) -> void :
	var p_timer_interval = _args[E.fs_project_change.p_timer_interval]
	if p_timer_interval == null:
		SB.public_set_int_value(500)
	else:
		SB.public_set_int_value(p_timer_interval)
...
func _on_value_changed(p_value: int) -> void :
	E.echo(E.ed_timer_interval_change, {
		E.ed_timer_interval_change.p_interval: p_value, })
